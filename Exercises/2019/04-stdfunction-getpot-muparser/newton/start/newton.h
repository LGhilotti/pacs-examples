...
newton_solver
{
 ...
 public:
  ...
  newton_solver (std::function<double (const double)> fun_,
                 ...
                 const double funtol_ = std::numeric_limits<double>::epsilon () * 1000.0, //a way to define the eps machine
                 const double xtol_ = std::numeric_limits<double>::epsilon () * 1000.0) //independently from the hardware.
